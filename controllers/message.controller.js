...
/**
 * @api {GET} /message/first Get First Message
 * @apiGroup Message
 * @apiName MessageGetFirstMessage
 *
 * @apiDescription Get the first message in a discussion.
 *
 ...
 */
const getFirstMessage = async (req, res, next) => {
  const discussionId = Number(req.query.discussionId);
  ...
  if (!message) {
    return next(new ErrorResponse('Cannot find message', httpStatus.NOT_FOUND, 'NOT_FOUND'));
  }
...
export { getMessagesInDiscussion, postMessage, updateMessage, deleteMessage, getFirstMessage, getMessage };
