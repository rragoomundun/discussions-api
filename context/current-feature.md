...
- **14-08-26 — Fix Sequelize Connection to Neon** — `utils/db.util.js` now disables Node's dual-stack `autoSelectFamily` (`net.setDefaultAutoSelectFamily(false)`) before connecting; a Node bug in that "Happy Eyeballs" logic caused spurious `ETIMEDOUT` against Neon's pooler endpoint even though the IP was reachable. Same fix applied to `migrations/package.json`'s `migration:migrate`/`migration:rollback` scripts via `NODE_OPTIONS=--no-network-family-autoselection`, plus added `production` variants of those scripts and SSL `dialectOptions` to `migrations/config/config.example.json`.
